...
void convert_to_ZBi_2D_QCD(TH2D* h2ZBi, TH2D* h2, TH2D* h2BG, TH2D* h2QCD, int nbinsx, double xlow, double xhigh, int nbinsy, double ylow, double yhigh) { 
  h2ZBi->Reset();
  TH2D* h2int = new TH2D("h2int","",    nbinsx,xlow,xhigh,nbinsy,ylow,yhigh);
  TH2D* h2BGint = new TH2D("h2BGint","",nbinsx,xlow,xhigh,nbinsy,ylow,yhigh);
  TH2D* h2QCDint = new TH2D("h2QCDint","",nbinsx,xlow,xhigh,nbinsy,ylow,yhigh);
  for (int x(0); x<nbinsx; x++) {
    for (int y(0); y<nbinsy; y++) {
      h2int->SetBinContent(x+1,y+1, h2->Integral(x+1,1000,y+1,1000));
      Double_t e_SM(0.), n_SM(0.);
      double n_qcd(0.);
      n_SM=h2BG->IntegralAndError(x+1,1000,y+1,1000,e_SM);
      n_qcd=h2QCD->Integral(x+1,1000,y+1,1000);
      // double fstat_bg = e_SM/n_SM;
      double fsyst_bg=n_qcd/n_SM; // ferr is QCD fraction (100 % uncertainty on QCD)
      // double ferr_bg = TMath::Sqrt(fstat_bg*fstat_bg+fsyst_bg*fsyst_bg);
      double ferr_bg = fsyst_bg;
      h2BGint->SetBinContent(x+1,y+1, n_SM);
      if (h2int->GetBinContent(x+1,y+1)>=3.0) h2ZBi->SetBinContent(x+1,y+1, RooStats::NumberCountingUtils::BinomialExpZ(h2int->GetBinContent(x+1,y+1),n_SM,ferr_bg));
    }
  }
  h2ZBi->SetMaximum(3);
  h2ZBi->SetMinimum(0);
