...
void mdpBinsOfMet(const TString var = "min_delta_phi_met_N", const TString btag_cut="num_csvm_jets30>=2&&num_jets_pt30>9", TString plotTitle="", bool logy = false, const TString treestring = "reduced_trees/QCD_Pt*v75*.root")
{ 
  ...
  TCut baseline ="ht30>=1000&&num_reco_veto_muons==0&&num_reco_veto_electrons==0"; //no mindp, no MET
  TString dp_cut(var);
  ...
  TCut LSB = "met<100";
  TCut MSB = "met>=100 && met<200";
  TCut SB = "met>=200 && met <400";
  TCut SIG = "met>400";
...
//   }
  ...
  char label[100];
  if (drawLSB)  {
    sprintf(label,"E_{T}^{miss} < 100 GeV (#mu=%3.2f)",Hlow->GetMean());
    leg->AddEntry(Hlow,label);
  }
  if (drawMSB)  {
    sprintf(label,"100 < E_{T}^{miss} < 200 GeV (#mu=%3.2f)",Hmed->GetMean());
    leg->AddEntry(Hmed,label);
  } 
  if (drawSB)   {
    sprintf(label,"200 < E_{T}^{miss} < 400 GeV (#mu=%3.2f)",Hmh->GetMean());
    leg->AddEntry(Hmh,label);
  }
  if (drawSIG)  {
    sprintf(label,"E_{T}^{miss} > 400 GeV (#mu=%3.2f)",Hhigh->GetMean());
...
//   Hlow->Chi2Test(Hmh,"WW p");
  ...
  if (plotTitle=="") {
    if(logy) {thecanvas->Print("macros/cutflow/plots/mdpMETcorrelation_"+var+"_logy.pdf");}
    else {thecanvas->Print("macros/cutflow/plots/mdpMETcorrelation_"+var+".pdf"); }
  }  
  else {
    if(logy) {thecanvas->Print("macros/cutflow/plots/"+plotTitle+"_logy.pdf");}
    else {thecanvas->Print("macros/cutflow/plots/"+plotTitle+".pdf"); }
  }
