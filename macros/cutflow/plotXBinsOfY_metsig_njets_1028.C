...
void plotXBinsOfY(const TString xvar = "num_jets_pt50", int nbinsx = 5, double xlow=4., double xhigh=14., const TString yvar = "met_over_sqrt_ht50", TString xAxisTitle="n_{jets} (p_{T} > 50 GeV)", TString yLegLabel = "E_{T}^{miss}/#sqrt{H_{T}}", const TString treestring = "reduced_trees/QCD*v75*.root", const TString plotNote="qcd", const TCut xtraCuts="", bool logy = false) 
{ 
  ...
  TCut baseline ="ht30>=1000&&num_reco_veto_muons==0&&num_reco_veto_electrons==0&&num_csvm_jets30>1&&num_jets_pt50>3";
  ...
  thecanvas->Print("macros/cutflow/plots/G3G/"+xvar+"_vs_"+yvar+"_"+plotNote+"_slices.pdf");
