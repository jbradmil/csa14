...
void lootpu_fit(TString files, TString comments="") {
  ...
  TH1F* hOOTPUvsIso = new TH1F("hOOTPUvsIso",";Late out-of-time ints.;Isolation cut efficiency", 11, 0, 44);
  ...
  for (int bin(1); bin<hOOTPUvsIso->GetNbinsX()+1; bin++) {
    ...
    TString cuts = Form("(%s)&&(loot_pu>=%d&&loot_pu<%d)",mu.Data(),low,low+4);
