...
void mjBinsOfNBM(const TString var = "MJ_pt100", TString plotTitle="", const TString treestring = "reduced_trees/QCD_Pt*v75*.root", bool logy = false)
{ 
  ...
  TCut baseline ="ht50>=500&&num_jets_pt50>2";
  TCut lep_veto("num_reco_veto_muons==0&&num_reco_veto_electrons==0");
  baseline = baseline+lep_veto;

  TCut LSB = "num_csvm_jets30==0";
  TCut MSB = "num_csvm_jets30==1";
  TCut SB = "num_csvm_jets30==2";
  TCut SIG = "num_csvm_jets30>=3";
  ...
  char label[100];
  if (drawLSB)  {
    sprintf(label,"0 CSVM (#mu=%3.2f)",Hlow->GetMean());
    leg->AddEntry(Hlow,label);
  }
  if (drawMSB)  {
    sprintf(label,"1 CSVM (#mu=%3.2f)",Hmed->GetMean());
    leg->AddEntry(Hmed,label);
  } 
  if (drawSB)   {
    sprintf(label,"2 CSVM (#mu=%3.2f)",Hmh->GetMean());
    leg->AddEntry(Hmh,label);
  }
  if (drawSIG)  {
    sprintf(label,"#geq 3 CSVM (#mu=%3.2f)",Hhigh->GetMean());
