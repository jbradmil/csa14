...
void mjBinsOfnj(const TString var = "MJ_pt100", TString plotTitle="", const TString treestring = "reduced_trees/QCD_Pt*v75*.root", bool logy = false)
{ 
  ...
  TCut LSB = "num_jets_pt50<=3";
  TCut MSB = "num_jets_pt50>3&&num_jets_pt50<=6";
  TCut SB = "num_jets_pt50>6&&num_jets_pt50<=9";
  TCut SIG = "num_jets_pt50>9";
  ...
  char label[100];
  if (drawLSB)  {
    sprintf(label,"0-3 jets (#mu=%3.2f)",Hlow->GetMean());
    leg->AddEntry(Hlow,label);
  }
  if (drawMSB)  {
    sprintf(label,"4-6 jets (#mu=%3.2f)",Hmed->GetMean());
    leg->AddEntry(Hmed,label);
  } 
  if (drawSB)   {
    sprintf(label,"7-9 jets (#mu=%3.2f)",Hmh->GetMean());
    leg->AddEntry(Hmh,label);
  }
  if (drawSIG)  {
    sprintf(label,"#geq 9 jets (#mu=%3.2f)",Hhigh->GetMean());
