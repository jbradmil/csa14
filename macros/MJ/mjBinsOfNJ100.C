...
void mjBinsOfNJ100(const TString var = "MJ_pt100", TString plotTitle="", const TString treestring = "reduced_trees/QCD_Pt*v75*.root", bool logy = false)
{ 
  ...
  float min=0;
  float max=0;
  if (treestring.Contains("QCD")) max=800.;
  else max = 2000.; 
  ...
  TCut baseline ="ht50>=500";
  TCut lep_veto("num_reco_veto_muons==0&&num_reco_veto_electrons==0");
  baseline = baseline+lep_veto;

  TCut LSB = "num_fat_jets_pt100<=2";
  TCut MSB = "num_fat_jets_pt100>2&&num_fat_jets_pt100<=4";
  TCut SB  = "num_fat_jets_pt100>4&&num_fat_jets_pt100<=6";
  TCut SIG = "num_fat_jets_pt100>6";
  ...
  char label[100];
  if (drawLSB)  {
    sprintf(label,"0-2 fat jets (#mu=%3.2f)",Hlow->GetMean());
    leg->AddEntry(Hlow,label);
  }
  if (drawMSB)  {
    sprintf(label,"3-4 fat jets (#mu=%3.2f)",Hmed->GetMean());
    leg->AddEntry(Hmed,label);
  } 
  if (drawSB)   {
    sprintf(label,"5-6 fat jets (#mu=%3.2f)",Hmh->GetMean());
    leg->AddEntry(Hmh,label);
  }
  if (drawSIG)  {
    sprintf(label,"#geq 7 fat jets (#mu=%3.2f)",Hhigh->GetMean());
