...
void DTTMHTRatioAnalysis::Run(TString outputtag, bool verb)
{
  ...
  for ( int pass=0; pass<2; pass++ ) {
	...
	if ( mbi >= 1 ) {
      ...
      //if ( pass == 0 && lost ) { h_lostlep_mht_mc_80bin -> Fill( MHT ) ;   }
	...
	if (selectedIDIsoElectronsNum==1) {
      ...
      if ( !((selectedIDIsoElectronsNum + selectedIDIsoMuonsNum) == 1) ) continue;
      if (mt_rec_mht>mt_cut) continue;
      ...
      if ( !(mu_trigger||el_trigger||lep_had_trigger) ) continue ;
      if (pass == 0) {
	...
	h_foundlep_wpt_mc_80bin[nbi][nji][hti] -> Fill( WpT_gen ) ;
      ...
      if ( verb ) printf( " %8llu : W pT rec from MHT %6.1f, wpt bin for rlf %d [%4.0f, %4.0f]\n", 
			  jentry, rec_w_pt_mht, wpti_for_rlf, wptbins_for_rlf[wpti_for_rlf], wptbins_for_rlf[wpti_for_rlf] ) ;
      if ( wpti_for_rlf < 0 ) continue ;
