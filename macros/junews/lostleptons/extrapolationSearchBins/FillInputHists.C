...
void FillInputHists::Run(TString outfiletag)
{
  ...
  TH1F* h_cdtt_all[5][5][20] ;
  TH1F* h_cdtt_lostlep[5][5][20] ;
  TH1F* h_cdtt_foundlep[5][5][20] ;
  ...
  for ( int nji=1; nji<nnjetbins; nji++ ) {
    for ( int hti=1; hti<nhtbins; hti++ ) {
      ...
      for ( int wpti=0; wpti<nwptbins_for_cdtt; wpti++ ) {

	sprintf( hname, "h_cdtt_all_nj%d_ht%d_wpt%d", nji, hti, wpti ) ;
	sprintf( htitle, "1/2(1-cos(dtt)), all, Njet%d, HT%d, Wpt%d", nji, hti, wpti ) ;
	h_cdtt_all[nji][hti][wpti] = new TH1F( hname, htitle, nbins_cdtt, xl_cdtt, xh_cdtt ) ;

	sprintf( hname, "h_cdtt_lostlep_nj%d_ht%d_wpt%d", nji, hti, wpti ) ;
	sprintf( htitle, "1/2(1-cos(dtt)), lost lepton, Njet%d, HT%d, Wpt%d", nji, hti, wpti ) ;
	h_cdtt_lostlep[nji][hti][wpti] = new TH1F( hname, htitle, nbins_cdtt, xl_cdtt, xh_cdtt ) ;

	sprintf( hname, "h_cdtt_foundlep_nj%d_ht%d_wpt%d", nji, hti, wpti ) ;
	sprintf( htitle, "1/2(1-cos(dtt)), found lepton, Njet%d, HT%d, Wpt%d", nji, hti, wpti ) ;
	h_cdtt_foundlep[nji][hti][wpti] = new TH1F( hname, htitle, nbins_cdtt, xl_cdtt, xh_cdtt ) ;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    h_cdtt_all[nji][hti][wpti] -> Fill( 0.5*(1.-cos_dTT) ) ;

    h_wpt_all[nji][hti] -> Fill( WpT ) ;

    if ( (selectedIDIsoElectronsNum + selectedIDIsoMuonsNum) == 0 ) {

      h_cdtt_lostlep[nji][hti][wpti] -> Fill( 0.5*(1.-cos_dTT) ) ;
	...
	rec_lep_pt=selectedIDIsoMuonsPt[0];
      ...
      if (MHT>mht_cut && mt_rec_mht<mt_cut && rec_lep_pt>lepton_pt_cut) {
	h_cdtt_foundlep[nji][hti][wpti] -> Fill( 0.5*(1.-cos_dTT) ) ;
