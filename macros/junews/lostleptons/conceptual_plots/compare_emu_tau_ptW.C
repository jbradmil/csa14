...
void compare_emu_tau_ptW(const TString treestring = "reduced_trees/13TeV/TTJets_MSDecaysCKM_central_Tune4C_13TeV-madgraph-tauola_Phys14DR-PU20bx25_PHYS14_25_V1-v1_MINIAODSIM_UCSB2406_v78/*.root", const TString plotNote="ttbar_2b", bool logy = true) {
  ...
  TH1D* hSUM = new TH1D("hSUM", ";Reconstructed p_{T}^{W} [GeV];Fraction of events", nbinsx, binsx);
  set_style(hMu, "qcd");
  ...
  double norm = hSUM->Integral(1, nbinsx);
  hMu->Scale(1/norm);
  hTAUMU->Scale(1/norm);
  hSUM->Scale(1/norm);
  ...
  c1->Print("macros/junews/lostleptons/conceptual_plots/compare_emu_tau_ptW_log.pdf");
